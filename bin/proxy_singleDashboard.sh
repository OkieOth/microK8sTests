...
# start default browser to access the dashboard ... with a little hack :D
sleep 1 && python -mwebbrowser http://localhost:9090/api/v1/namespaces/kube-system/services/https:kubernetes-dashboard:/proxy/#/login &
...
kubectl --kubeconfig=$scriptPos/../kubectlConfig/single-microk8s --user=admin \
    ...
    proxy --address="0.0.0.0" --port=9090
