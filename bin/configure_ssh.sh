...
keyFileName=${configDest}/ssh/mircoK8sTest_${CONFIG_DEST}

if ! [[ -f $keyFileName ]]; then
    ssh-keygen -f "$keyFileName" -C "mircoK8sTest: ${SSH_USER}@${SSH_HOST}" -N ""
else
    echo "SSH key file already exists: $keyFileName"
fi
